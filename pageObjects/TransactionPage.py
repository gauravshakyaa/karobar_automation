...
class CreateTransactionPage: # This includes Sales, Sales Return, Purchase, Purchase Return and Quotation
    ...
    searchField_itemNAme_xpath = f"//tr[{itemTableColumnIndex}]//input[@class='p-3 w-full h-full rounded-none outline-none bg-background hover:bg-secondary focus:ring-focus focus:ring-1 [&amp;::-webkit-outer-spin-button]:appearance-none [&amp;::-webkit-inner-spin-button]:appearance-none relative min-w-[25rem]']"
    inputField_quantity_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[2]/div/input"
    inputField_rate_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[3]/input"
    inputField_discountPercent_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[4]/div/div[1]//input"
    inputField_discountAmount_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[4]/div/div[2]//input"
    inputField_billingAmount_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[5]//input"
    button_deleteBillingItem_xpath = f"//tbody/tr[{itemTableColumnIndex}]/td[5]/span/button"
    ...
    def setParty(self, name):
        try:
            ...
            conftest.sendKeys(self.driver, self.searchField_selectParty_name, By.XPATH, name)
    ...
    def setInvoiceNumber(self, invoice_number):
        try:
            ...
            conftest.sendKeys(self.driver, self.inputField_invoice_xpath, by = By.XPATH, value = invoice_number)
        except Exception as e:
